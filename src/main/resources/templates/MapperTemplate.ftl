  ...
  <insert id="insert" parameterType="${bussPackage}.${entityPackage}.entity.${className}" >
     ...
     <selectKey resultType="${pkColumn.dataType}" order="BEFORE" keyProperty="${pkColumn.columnName}">
      ...
      <trim prefix="values (" suffix=")" suffixOverrides="," >
          #foreach($po in $!{columnDatas})
          <if test="$!{po.dataName} != null" >#{$!{po.dataName}},</if>
          #end
      </trim>
  </insert>
...
#foreach($po in ${pkColumnDatas})
...
#end
  <!-- ${codeName} 列表总数-->
  <select id="queryByCount" resultType="java.lang.Integer" parameterType="com.ebiz.java.util.base.page.BasePage">
    select count(1) from ${tableName}
    <include refid="Base_Where_Clause"/>
  </select>

  <!-- 查询${codeName}列表 -->
  <select id="queryByPage" resultMap="BaseResultMap" parameterType="com.ebiz.java.util.base.page.BasePage">
  ...
  <select id="queryByList" resultMap="BaseResultMap" parameterType="com.ebiz.java.util.base.page.BasePage">
    select <include refid="Base_Column_List"/> from ${tableName}
    <include refid="Base_Where_Clause"/>
  </select>
